        ...
        foreach ($configs as $section => $configOptions) {
            ...
            foreach ($configOptions as $configName => $configValue) {
                ...
                if ($update and !empty($currentValue)) {
                    ...
                    $newConfigOptions[$configName] = $dialog->askAndValidate($output, $dialog->getQuestion(sprintf('%s', $configName), $configValue), array($validate, 'notEmpty'), false, $configValue);
                    $newConfigRead = true;
                }
            }
